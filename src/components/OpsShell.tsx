...
export function OpsShell({ children }: { children: ReactNode }) {
  return <div className="ops-shell"><aside className="ops-sidebar"><p className="eyebrow">Demo workspace</p><h2>Operations</h2><div className="role-pill"><Activity size={15} /> Operator role</div><nav aria-label="Operations navigation">{nav.map(([label, href, Icon]) => <Link href={href} key={href}><Icon size={18} />{label}</Link>)}</nav><div className="sidebar-note"><strong>Safe demo</strong><p>All records are synthetic. Changes persist only in this browser.</p></div></aside><section className="ops-main" aria-label="Operations workspace">{children}</section></div>;
}
